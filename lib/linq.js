...
function Linq(dbName) {
    ...
    function resolveWhereExpressionTree(tree, not) {
        ...
        if (tree.operator) {
            ...
            if (left) {
                if (not) {
                ...
                else if (tree.operator === "=" && right === null) {
                    return `${left} IS NULL`
                }
                else if (tree.operator === "<>" && right === null) {
                    ...
                    return `${left} ${tree.operator} ${right === null ? "NULL" : right}`
                }
            }
            else {
                return `${right}`
            ...
            if (tree.value === null || tree.value === undefined) {
                return null;
            }
    ...
    function resolveField(field) {
        ...
        if (field.operator === "set" && field.left.type === "var" && field.right.type === "field") {
            return [field.right.value, 'as', field.left.value].join(" ");
        }
        throw new TypeError("Unsupported syntax:" + JSON.stringify(field))
    }
    ...
    function resolveWhere(useAlias) {
        var object_afx = [];
        if (tables[0].alias) {
            ...
            if (Array.isArray(w)) {
                ...
                for (var field in w) {
                    if (Array.isArray(w[field])) {
                        _wheres.push(`${object_afx.join('.')}.${expression.formater(field)} in (${escaper(w[field])})`);
                    }
                    else {
                        if (w[field] === null || w[field] === undefined) {
                            _wheres.push(`${object_afx.join('.')}.${expression.formater(field)} IS NULL`);
                        }
                        else {
                            _wheres.push(`${object_afx.join('.')}.${expression.formater(field)}=${escaper(w[field])}`);
                        }
                    }
     ...
     * @param {object|Function} replacer - 当插入的数据存在主键冲突时，使用此对象进行替换
     * @param {object} consts - 当repplacer是一个表达式时，此处是表达式中需析常量值。
     * @returns string
     */
    this.toInsertSql = function (inserter, replacer, consts) {
        var sql = [], tableName = [];
        var firstTable = tables[0];
        if (firstTable.db) {
            tableName.push(expression.formater(firstTable.db));
        ...
        if (replacer && Array.isArray(inserter)) {
            throw new Error('When the replacer parameter is included, inster can only be an object.');
        }
        sql.push('INSERT INTO')
        var fields = [], values = [], sets = [];
        if (typeof inserter === "object") {
            ...
            for (var i = 0; i < inserter.length; i++) {
                ...
                for (var j = 0; j < __feilds.length; j++) {
                    vFields.push(escaper(item[__feilds[j]]));
                }
            ...
            if (replacer) {
                ...
                if (typeof replacer === "object") {
                    for (var field in replacer) {
                        sets.push(`${expression.formater(field)}=${escaper(replacer[field])}`)
                    }
                }
                else if (typeof replacer === "function") {
                    sets = resolveSet(replacer, consts);
                }
                // if (sets.length) {
                //     sql.push(sets.join(','));
                // }
