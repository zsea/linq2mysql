 ...
 * @returns object
 */
Linq.prototype.insert = function (insertor,replacer,consts) {
    var sql = this.toInsertSql(insertor,replacer,consts);
    return this.execute(sql);
}
/**
 * 统计数量
...
function db(connectionStringOrconnectionObject, logger, isTrans) {
    ...
    if (typeof connectionStringOrconnectionObject === "object") {
        ...
        connectObject["multipleStatements"] = connectObject["multipleStatements"] === "true";
    }
    else {
        throw Error("连接错误。")
    }
        ...
        if (connectObject.supportBigNumbers && !connectObject.bigNumberStrings && !connectObject["typeCast"]) {
            connectObject["typeCast"] = function (field, next) {
                ...
                if (field.type === 'LONGLONG') {
                    //console.log(field)
                    var v=field.string();
                    //console.log("====",field.name,field.type,v);
                    if(v){
                        return BigInt(v);
                    }
                    else{
